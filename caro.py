...
class Window(tk.Tk):
    ...
    def showFrame(self):
        ...
        Undo = tk.Button(frame1, text="Undo", width=10, background="green",  # nút quay lại
        ...
        connectBT = tk.Button(frame1, text="Connect", width=10, background="blue",
                              command=lambda: self.Threading_socket.clientAction(inputIp.get()))
        connectBT.grid(row=0, column=3, padx=3)

        makeHostBT = tk.Button(frame1, text="MakeHost", width=10, background="Yellow",  # nút tạo host
                               command=lambda: self.Threading_socket.serverAction())
        makeHostBT.grid(row=0, column=4, padx=30)

        Playbot = tk.Button(frame1, text="Chơi với máy", width=10, background="red",  # Chơi với máy
                               command=lambda: self.Threading_socket.serverAction())
        Playbot.grid(row=0, column=5, padx=30)
    ...
    def handleButton(self, x, y):
        if self.Buts[x, y]['text'] == "": #Kiểm tra ô có ký tự rỗng hay không
            ...
            if len(self.memory) % 2 == 1:
                ...
                self.Buts[x, y]['foreground'] = 'blue'  # Thiết lập màu chữ cho O
                ...
                self.Buts[x, y]['foreground'] = 'red'  # Thiết lập màu chữ cho X
                self.Threading_socket.sendData("{}|{}|{}|".format("hit", x, y))
...
if __name__ == "__main__":
    Ox = 30  # Số lượng ô theo trục X
    Oy = 30 # Số lượng ô theo trục Y
    window = Window()
    window.showFrame()
    window.mainloop()